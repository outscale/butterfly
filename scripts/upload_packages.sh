...
if [ "${path: -1}" != "/" ] ; then
    echo -e "Error: path should end with '/'"
    usage
    exit 1
fi
...
for p in *.rpm *.deb ; do
    s3cmd put --acl-public --access_key=$ak --secret_key=$sk --host=$endpoint --host-bucket='%(bucket).$e' $p $path${prefix}_$p
done
...
for p in *.rpm *.deb ; do
    s3cmd put --acl-public --access_key=$ak --secret_key=$sk --host=$endpoint --host-bucket='%(bucket).$e' $p $path${prefix}_fat_$p
done
