...
static void firewall_filter_rules(enum side dir)
{
	...
	ret = firewall_rule_add(fw, "src host 10.0.0.1", dir, 0);
	g_assert(ret == 0);
	ret = firewall_reload(fw);
	g_assert(ret == 0);
	...
	ret = firewall_rule_add(fw, "src host 10.0.0.2", dir, 0);
	g_assert(ret == 0);
	ret = firewall_reload(fw);
	g_assert(ret == 0);
	...
	ret = firewall_rule_flush(fw);
	g_assert(ret == 0);
	ret = firewall_rule_add(fw, "src host (10.0.0.1)", MAX_SIDE, 0);
	g_assert(ret == 0);
	ret = firewall_reload(fw);
	g_assert(ret == 0);
			    ...
			    int pkts_nb, int *pkts_size)
{
	 ...
	 */
	ret = firewall_rule_add(fw, "src host 10.0.2.15", WEST_SIDE, 1);
	g_assert(ret == 0);
	ret = firewall_reload(fw);
	g_assert(ret == 0);
