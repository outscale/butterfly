...
bool API_0::from_to(const app::Nic &nic_model, MessageV0_Nic *nic_message) {
...
bool API_0::from_to(const MessageV0_Nic &nic_message, app::Nic *nic_model) {
    if (nic_model == nullptr)
        return false;
    // Id
    nic_model->id = nic_message.id();
    // MAC
    if (!from_to(nic_message.mac(), &nic_model->mac))
        return false;
    // VNI
    ...
    for (int a = 0; a < nic_message.ip_size(); a++) {
        app::Ip ip;
        if (!from_to(nic_message.ip(a), &ip))
            return false;
...
bool API_0::from_to(const MessageV0_NicUpdateReq &nic_update_message,
    ...
    for (int a = 0; a < nic_update_message.ip_size(); a++) {
        app::Ip ip;
        if (!from_to(nic_update_message.ip(a), &ip))
            return false;
...
bool API_0::from_to(const app::Sg &sg_model, MessageV0_Sg *sg_message) {
...
bool API_0::from_to(const MessageV0_Sg &sg_message, app::Sg *sg_model) {
    if (sg_model == nullptr)
        return false;
    // Id
    ...
    for (int a = 0; a < sg_message.member_size(); a++) {
        app::Ip ip;
        if (!from_to(sg_message.member(a), &ip))
            return false;
    ...
    for (int a = 0; a < sg_message.rule_size(); a++) {
        app::Rule rule;
        if (!from_to(sg_message.rule(a), &rule))
            return false;
...
bool API_0::from_to(const app::Rule &rule_model,
    ...
    if (rule_model.security_group.length() > 0) {
        ...
        if (!from_to(rule_model.cidr, cidr))
            return false;
    }
    return true;
}

bool API_0::from_to(const MessageV0_Rule &rule_message, app::Rule *rule_model) {
    ...
    if (rule_message.has_security_group()) {
        ...
        if (!from_to(rule_message.cidr(), &rule_model->cidr))
            return false;
    } else {
        return false;
    }
...
bool API_0::from_to(const app::Ip &ip_model, std::string *ip_message) {
...
bool API_0::from_to(const std::string &ip_message, app::Ip *ip_model) {
    if (ip_model == nullptr)
        return false;
    *ip_model = ip_message;
    return true;
...
bool API_0::from_to(const app::Mac &mac_model, std::string *mac_message) {
...
bool API_0::from_to(const std::string &mac_message, app::Mac *mac_model) {
...
bool API_0::from_to(const app::Cidr &cidr_model, MessageV0_Cidr *cidr_message) {
    if (cidr_message == nullptr)
        return false;
    // Address
    auto ip = cidr_message->mutable_address();
    if (!from_to(cidr_model.address, ip))
        return false;
    // Address mask
...
bool API_0::from_to(const MessageV0_Cidr &cidr_message, app::Cidr *cidr_model) {
    if (cidr_model == nullptr)
        return false;
    // Address
    if (!from_to(cidr_message.address(), &cidr_model->address))
        return false;
    // Address mask
...
bool API_0::from_to(const app::Error &error_model,
