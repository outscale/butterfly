...
Each nick have a 24 bits network id call VNI.
If two vnics with the same VNI are located on different physical hosts,
Butterfly will encapsulate VM packets over VXLAN and send it to the
corresponding physical host.
Once received, the packet will be decapsulated and routed to it's final
destination.
...
For VM to VM communication, checksum and segmentation won't occure as the packet
...
A SG contains a list of rules to open (default policy is to block) and a list
of members (IP addresses).

A rule is mainly described by a protocol/port to allow and a source to allow.
...
e.g. dd rule in security group "mysg" to allow members of SG "users":
on TCP port 80:
```
