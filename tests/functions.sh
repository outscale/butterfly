...
function update_sg_rule_port_open {
    ...
    sg=$5
    f=/tmp/butterfly-client.req
    echo "update nic $nic_id with only port $port opened in butterfly $but_id in vni $vni"
...
function delete_rule_full_open {
    ...
    nic_id=$2
    sg=$3
    echo "delete nic $nic_id rules full open in butterfly $but_id"
    f=/tmp/butterfly-client.req
